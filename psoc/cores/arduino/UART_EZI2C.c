...
***************************************/
    ...
    ****************************************************************************//**
    *
    *  Configures the UART for EZI2C operation.
    *
    ...
    *  After initializing the UART in EZI2C mode using this function, 
...
*  - UART_EZI2C_STATUS_READ1 - Read transfer complete. The transfer 
...
*  - UART_EZI2C_STATUS_WRITE1 - Write transfer complete. The buffer 
...
*  - UART_EZI2C_STATUS_READ2 - Read transfer complete. The transfer
*    used the secondary slave address. The error condition status bit must be 
*    checked to ensure that read transfer was completed successfully.
*  - UART_EZI2C_STATUS_WRITE2 - Write transfer complete. The buffer
*    content was modified. The transfer used the secondary slave address. 
*    The error condition status bit must be checked to ensure that write 
*    transfer was completed successfully.
*  - UART_EZI2C_STATUS_BUSY - A transfer intended for the primary 
...
*  - UART_EZI2C_STATUS_ERR - An error occurred during a transfer 
...
*    UART_EZI2C_STATUS_ERR and UART_EZI2C_STATUS_WRITE1/2 
*    is set. It is recommended to discard buffer content in this case.
*
* \globalvars
*  UART_curStatus - used to store the current status of the EZI2C 
*  slave.
*
*******************************************************************************/
...
*
* \globalvars
*  UART_addrBuf1 - used to store the primary 7-bit slave address 
*  value.
*
*******************************************************************************/
void UART_EzI2CSetAddress1(uint32 address)
{
    if(UART_SECONDARY_ADDRESS_ENABLE)
    {
        ...
        UART_RX_MATCH_REG = 
                        UART_EzI2CUpdateRxMatchReg(address, (uint32) UART_addrBuf2);
    }
    else
    {
        uint32 matchReg;
        ...
        /* Set address. */
        matchReg &= (uint32) ~UART_RX_MATCH_ADDR_MASK;
...
*
* \globalvars
*  UART_dataBuffer1 – the pointer to the buffer to be exposed to the
*  master on a primary address.
*  UART_bufSizeBuf1 - the size of the buffer to be exposed to the 
*  master on a primary address.
*  UART_protectBuf1 - the start index of the read-only region in the
...
void UART_EzI2CSetReadBoundaryBuffer1(uint32 rwBoundary)
{
    ...
    *
    * \globalvars
    *  UART_addrBuf2 - used to store the secondary 7-bit slave address 
    *  value.
    *
    *******************************************************************************/
    void UART_EzI2CSetAddress2(uint32 address)
    {
        ...
        UART_RX_MATCH_REG = 
                        UART_EzI2CUpdateRxMatchReg((uint32) UART_addrBuf1, address);
    }
    ...
    *
    * \globalvars
    *  UART_dataBuffer2 – the pointer to the buffer to be exposed to the
    *  master on a secondary address.
    *  UART_bufSizeBuf2 - the size of the buffer to be exposed to the 
    *  master on a secondary address.
    *  UART_protectBuf2 - the start index of the read-only region in the
