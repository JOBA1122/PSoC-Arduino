...
*  Enables the I2C interrupt and clears UART_I2C_MSTAT_WR_CMPLT 
*  status.
*
...
*  - UART_I2C_MODE_COMPLETE_XFER - Perform complete transfer 
*    from Start to Stop.
*  - UART_I2C_MODE_REPEAT_START - Send Repeat Start instead 
*    of Start. A Stop is generated after transfer is completed unless 
*    NO_STOP is specified.
*  - UART_I2C_MODE_NO_STOP Execute transfer without a Stop.
*    The following transfer expected to perform ReStart.
*
* \return
*  Error status.
*  - UART_I2C_MSTR_NO_ERROR - Function complete without error. 
*    The master started the transfer.
*  - UART_I2C_MSTR_BUS_BUSY - Bus is busy. Nothing was sent on
*    the bus. The attempt has to be retried.
*  - UART_I2C_MSTR_NOT_READY - Master is not ready for to start
...
*  Once the data transfer is initiated by this function, further data transfer
*  is handled by the included ISR.
*  Enables the I2C interrupt and clears UART_I2C_MSTAT_RD_CMPLT 
*  status.
*
*  \param slaveAddr: 7-bit slave address.
*  \param xferData: Pointer to buffer of data to be sent.
...
*  be ORed together. See UART_I2CMasterWriteBuf() 
*  function for constants.
*
* \return
*  Error status.See UART_I2CMasterWriteBuf() 
*  function for constants.
*
* \globalvars
*  UART_mstrStatus  - used to store current status of I2C Master.
...
*  UART_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  UART_mstrRdBufPtr - used to store pointer to master read buffer.
*  UART_mstrRdBufIndex - used to current index within master read
*  buffer.
*  UART_mstrRdBufSize - used to store master read buffer size.
*
*******************************************************************************/
...
*  - UART_I2C_WRITE_XFER_MODE - Set write direction for the 
*    following transfer.
*  - UART_I2C_READ_XFER_MODE - Set read direction for the 
*    following transfer.
*
* \return
*  Error status.
*   - UART_I2C_MSTR_NO_ERROR - Function complete without error.
*   - UART_I2C_MSTR_BUS_BUSY - Bus is busy. 
*     Nothing was sent on the bus. The attempt has to be retried.
*   - UART_I2C_MSTR_NOT_READY - Master is not ready for to 
*     start transfer.
...
*     Nothing was sent on the bus. The attempt has to be retried.
*   - UART_I2C_MSTR_ERR_LB_NAK - Error condition: Last byte was 
*     NAKed.
*   - UART_I2C_MSTR_ERR_ARB_LOST - Error condition: Master lost 
*     arbitration.
*   - UART_I2C_MSTR_ERR_BUS_ERR - Error condition: Master 
*     encountered a bus error. Bus error is misplaced start or stop detection.
*   - UART_I2C_MSTR_ERR_ABORT_START - Error condition: The start 
...
*  See UART_I2CMasterSendStart() function for constants.
*
* \return
*  Error status. 
*  See UART_I2CMasterSendStart() function for constants.
...
*  UART_I2CMasterReadBuf() function.
*
* \return
...
*  UART_I2C_MSTAT_ERR_ARB_LOST or
*  UART_I2C_MSTAT_ERR_BUS_ERROR occurred while read transfer.
*
* \globalvars
*  UART_mstrRdBufIndex - used to current index within master read
*  buffer.
...
*  - UART_I2C_MSTAT_ERR_XFER - Error condition: This is the 
...
*
* \return
*  Current status of master. See the UART_I2CMasterStatus() 
*  function for constants.
*
* \globalvars
*  UART_mstrStatus - used to store current status of I2C Master.
