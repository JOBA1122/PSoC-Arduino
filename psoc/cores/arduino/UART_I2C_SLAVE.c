...
*   - UART_I2C_SSTAT_RD_CMPLT - Slave read transfer complete. 
...
*   - UART_I2C_SSTAT_RD_BUSY - Slave read transfer is in progress. 
*     Set when master addresses slave with a read, cleared when 
*    UART_I2C_SSTAT_RD_CMPLT is set.
*   - UART_I2C_SSTAT_RD_OVFL - Master attempted to read more bytes
*     than are in buffer. Slave continually returns 0xFF byte in this case.
*   - UART_I2C_SSTAT_RD_ERR - Slave captured error on the bus during 
*     a read transfer. The sources of error are: misplaced Start or Stop 
*     condition or lost arbitration while slave drives SDA.
*   - UART_I2C_SSTAT_WR_CMPLT - Slave write transfer complete. 
...
*     successfully.
*   - UART_I2C_SSTAT_WR_BUSY - Slave write transfer is in progress. 
*     Set when the master addresses the slave with a write, cleared when 
*    UART_I2C_SSTAT_WR_CMPLT is set.
*   - UART_I2C_SSTAT_WR_OVFL - Master attempted to write past end of 
*     buffer. Further bytes are ignored.
*   - UART_I2C_SSTAT_WR_ERR - Slave captured error on the bus during
...
*     UART_I2C_SSTAT_WR_ERR is set. It is recommended to discard write 
...
*
* \sideeffect
*  This function does not clear UART_I2C_SSTAT_RD_BUSY.
*
* \globalvars
*  UART_slStatus  - used to store current status of I2C slave.
*
...
*
* \sideeffect
*  This function does not clear UART_I2C_SSTAT_WR_BUSY.*
*
* \globalvars
*  UART_slStatus  - used to store current status of I2C slave.
*
...
void UART_I2CSlaveSetAddressMask(uint32 addressMask)
{
    ...
    matchReg &= ((uint32) ~UART_RX_MATCH_MASK_MASK);
...
*  UART_SlaveGetReadBufSize() function.
*
...
*  resets the transfer count returned with the 
*  UART_I2CSlaveGetWriteBufSize() function.
*
...
*  UART_I2CSlaveInitReadBuf() or 
*  UART_I2CSlaveClearReadBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if UART_I2C_SSTAT_RD_ERR was 
...
*  UART_I2CSlaveInitWriteBuf() or 
*  UART_I2CSlaveClearWriteBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if UART_I2C_SSTAT_WR_ERR was 
*  captured by the slave.
*
* \globalvars
*  UART_slWrBufIndex - used to store current index within slave
