...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        PIN7_PC =   (PIN7_PC & \
                                (uint32)(~(uint32)(PIN7_DRIVE_MODE_IND_MASK << (PIN7_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (PIN7_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(PIN7_DR & (uint8)(~PIN7_MASK));
    drVal = (drVal | ((uint8)(value << PIN7_SHIFT) & PIN7_MASK));
    PIN7_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(PIN7__0__SHIFT, mode);
}
...
{
    return (uint8)((PIN7_PS & PIN7_MASK) >> PIN7_SHIFT);
}
...
{
    return (uint8)((PIN7_DR & PIN7_MASK) >> PIN7_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(PIN7_INTSTAT & PIN7_MASK);
		PIN7_INTSTAT = maskedStatus;
        return maskedStatus >> PIN7_SHIFT;
    }
